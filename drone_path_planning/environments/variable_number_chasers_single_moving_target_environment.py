...
class VariableNumberChasersSingleMovingTargetEnvironment(VariableNumberAgentEnvironment):
    def __init__(
        self,
    ...
    def reset(self):
        ...
        self._chaser_displacements = self._create_initial_chaser_displacements(1, center_x=center_x, center_y=center_y)
        for _ in range(1, num_chasers):
            chaser_displacement = self._create_initial_chaser_displacements(1, center_x=center_x, center_y=center_y)
        ...
        num_targets = 1
        self._target_displacements = self._create_initial_target_displacements(1, center_x=center_x, center_y=center_y)
    ...
    def _create_initial_chaser_displacements(self, num_chasers: int, center_x: float = 0.0, center_y: float = 0.0) -> tf.Tensor:
        return self._create_initial_displacements(num_chasers, center_x=center_x, center_y=center_y)

    def _create_initial_target_displacements(self, num_targets: int, center_x: float = 0.0, center_y: float = 0.0) -> tf.Tensor:
        return self._create_initial_displacements(num_targets, center_x=center_x, center_y=center_y)

    def _create_initial_displacements(self, num_entities: int, center_x: float = 0.0, center_y: float = 0.0) -> tf.Tensor:
        half_initial_bounds_width = _INITIAL_BOUNDS_WIDTH / 2.
        half_initial_bounds_height = _INITIAL_BOUNDS_HEIGHT / 2.
        ...
        displacements = tf.concat(
            [
                tf.random.uniform(
                    [num_entities, 2],
                    minval=tf.constant([[min_x, min_y]]),
                    maxval=tf.constant([[max_x, max_y]]),
                ),
